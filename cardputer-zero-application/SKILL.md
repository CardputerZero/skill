...
description: Build, port, package, review, validate, or debug CardputerZero applications that run through APPLaunch or are generated by VibAPP. Use when working with CardputerZero app projects, APPLaunch `.desktop` entries, `/usr/share/APPLaunch`, LVGL 9.5 framebuffer or SDL builds, keyboard and CJK input, AArch64 packaging, Debian `.deb` staging, on-device installation, or APPLaunch registration failures.
metadata:
  short-description: Build and package CardputerZero APPLaunch apps
---

# CardputerZero Applications
...
Hard rule for on-device work: when the current host is the CardputerZero, copy the final `.desktop`, wrapper/binary, support files, and icon into these paths and reload APPLaunch before finishing. The success criterion is that the customer can navigate the launcher, see the app entry with a logo, and launch it.
...
The template includes a prebuilt LVGL component library plus headers, so it does not require the 800MB SDK on the CardputerZero. If `make`, `g++`, or the template path is missing, fail clearly instead of inventing fake LVGL files or empty SConstruct projects.
...
When running the helper on the CardputerZero itself, install to APPLaunch immediately:
...
- Framebuffer app uses Linux fbdev, can find `fb_st7789v`, and does not assume `/dev/fb0`; verify the CardputerZero LCD path, often `/dev/fb1`.
...
- On CardputerZero, generated files were actually copied under `/usr/share/APPLaunch` and APPLaunch was restarted/reloaded.
